...
export default function Home() {
  ...
  // 1. قائمة الرحلات أو الكورسات المسجلة
  const [myBookedTrips, setMyBookedTrips] = useState<any[]>([]);

  // 2. قائمة المدفوعات والفواتير المرتبطة بالعميل
  const [paymentsList, setPaymentsList] = useState<any[]>([]);

  // محاكاة حجز رحلة جديدة أو اشتراك في كورس وإضافتها لـ "رحلاتي/كورساتي" و "المدفوعات"
  const handleBookingSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const newTripId = Date.now();
    
    const isCourse = selectedDestination.includes("كورس");
    const newTrip = {
      id: newTripId,
    ...
    // تحويل المستخدم تلقائياً لقسم "رحلاتي" لرؤية تفاصيل حجزه
    setActiveTab('my-trips');
  };

  // محاكاة الدفع عبر بوابة الدفع الإلكتروني
  ...
  return (
    <div className="min-h-screen bg-[#F7F3EA] text-[#172126] font-sans relative" dir="rtl">
      ...
      <header className="bg-[#073B4C] text-white shadow-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex justify-between items-center">
          ...
          <nav className="hidden lg:flex gap-4 font-medium text-sm">
            ...
            <button onClick={() => setActiveTab('biology')} className={`transition pb-1 ${activeTab === 'biology' ? 'text-[#19B5A5] border-b-2 border-[#19B5A5] font-bold' : 'hover:text-[#19B5A5]'}`}>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <a 
              href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent("مرحباً، أريد الاستفسار عن خدمات بداية ترافيل أو كورسات أ/ مروان الجندي للأحياء.")}`} 
              target="_blank" 
      ...
      <section className="relative bg-[#006B7A] text-white py-12 sm:py-16 px-4 sm:px-6 text-center overflow-hidden">
        <div className="max-w-4xl mx-auto z-10 relative">
          ...
          <p className="text-sm sm:text-lg text-gray-200 mb-6 leading-relaxed">
            استكشف أجمل الوجهات السياحية في مصر، وتابع أقوى كورسات وفيديوهات العلوم والأحياء مع <span className="text-amber-300 font-bold">أ/ مروان الجندي</span>.
          </p>
          ...
          <div className="flex justify-center gap-3 flex-wrap">
            ...
            <button onClick={() => setActiveTab('biology')} className={`px-5 py-2.5 rounded-full font-bold text-sm transition shadow ${activeTab === 'biology' ? 'bg-[#FF7A59] text-white' : 'bg-white/20 hover:bg-white/30 text-white'}`}>
              🎥 كورسات أ/ مروان الجندي
            </button>
                ...
                {myBookedTrips.map((myTrip) => (
                  <div key={myTrip.id} className="bg-[#F7F3EA] rounded-2xl p-5 border border-gray-200 shadow flex flex-col justify-between">
                    ...
                    <div className="space-y-2">
                      ...
                      <a 
                        href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`مرحباً، أود الاستفسار وتأكيد تفاصيل اشتراكي/حجزي: ${myTrip.title}`)}`}
                        target="_blank" 
                        rel="noopener noreferrer"
                ...
                <div>
                  ...
                  >
                    ...
                    <optgroup label="كورسات أ/ مروان الجندي للأحياء والعلوم">
                      <option value="كورس: الأحياء العامة - الثانوية العامة (الفصل الأول)">كورس: الأحياء العامة - الثانوية العامة (الفصل الأول)</option>
                      <option value="كورس: مراجعة ليلة الامتحان في الأحياء والعلوم">كورس: مراجعة ليلة الامتحان في الأحياء والعلوم</option>
                      <option value="كورس: أساسيات علم الأحياء والوراثة بالتفصيل">كورس: أساسيات علم الأحياء والوراثة بالتفصيل</option>
                    </optgroup>
                  </select>
                </div>
                <button 
      ...
      <a
        href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent("مرحباً، أريد الاستفسار عن تفاصيل الرحلات أو كورسات أ/ مروان الجندي للأحياء.")}`}
        target="_blank"
        rel="noopener noreferrer"
